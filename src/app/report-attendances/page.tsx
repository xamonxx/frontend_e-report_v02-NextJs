        ...
        <>
          ...
          {statusCounts && (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
              <div className="border border-border bg-card/30 p-3 rounded-xl dark:border-zinc-800 dark:bg-zinc-900/30">
                <p className="text-[10px] font-bold text-muted-foreground/70 uppercase">Total Admin</p>
                <p className="text-lg font-extrabold text-foreground/90 mt-1">{statusCounts.all}</p>
              </div>
              <div className="border border-border bg-card/30 p-3 rounded-xl dark:border-zinc-800 dark:bg-zinc-900/30">
                <p className="text-[10px] font-bold text-green-600 uppercase dark:text-green-400">Ada WA</p>
                <p className="text-lg font-extrabold text-green-600 mt-1 dark:text-green-400">{statusCounts.ada_wa}</p>
              </div>
              <div className="border border-border bg-card/30 p-3 rounded-xl dark:border-zinc-800 dark:bg-zinc-900/30">
                <p className="text-[10px] font-bold text-amber-500 uppercase font-mono">0 WA</p>
                <p className="text-lg font-extrabold text-amber-500 mt-1">{statusCounts.nol_wa}</p>
              </div>
              <div className="border border-border bg-card/30 p-3 rounded-xl dark:border-zinc-800 dark:bg-zinc-900/30">
                <p className="text-[10px] font-bold text-blue-600 uppercase dark:text-blue-400">Libur</p>
                <p className="text-lg font-extrabold text-blue-600 mt-1 dark:text-blue-400">{statusCounts.libur_susulan}</p>
              </div>
              <div className="border border-border bg-card/30 p-3 rounded-xl dark:border-zinc-800 dark:bg-zinc-900/30">
                <p className="text-[10px] font-bold text-red-500 uppercase dark:text-red-400">Belum Lapor</p>
                <p className="text-lg font-extrabold text-red-500 mt-1 dark:text-red-400">{statusCounts.belum_laporan}</p>
              </div>
            </div>
          )}

          {/* Quick Category filter tabs */}
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between border-b border-border pb-3 dark:border-zinc-800">
            <div className="flex gap-1.5 overflow-x-auto pb-px">
              {[
                { id: 'all', label: 'Semua Admin' },
                { id: 'ada_wa', label: 'Ada WA' },
                { id: 'nol_wa', label: '0 WA' },
                { id: 'libur_susulan', label: 'Libur' },
                { id: 'belum_laporan', label: 'Belum Laporan' }
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setSelectedStatus(tab.id)}
                  className={cn(
                    "px-4 py-2 text-xs font-semibold focus:outline-none border-b-2 transition-all shrink-0",
            ...
            <div className="flex items-center gap-2">
              {(['PC', 'NPP'] as const).map((group) => (
                <a
                  key={group}
                  href={buildExportUrl('/api/v1/report-attendances/export', {
                    date: selectedDate,
                    account_group: group,
                  })}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-card text-foreground/80 hover:bg-muted text-xs font-semibold transition-colors dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800"
                >
